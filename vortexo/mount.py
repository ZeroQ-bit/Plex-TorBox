...
class MountSupervisor:
    ...
    def validate_storage(self):
        ...
        if _is_mountpoint(self.mountpoint):
            ...
            raise RuntimeError(
                f"Another service already owns {self.mountpoint} ({fs_type}); stop it before Plex TorBox"
            )
        try:
    ...
    def _recover_stale_owned_mount(self):
        ...
        if not os.path.isfile(self.owner_marker):
            raise RuntimeError(
                "A disconnected mount exists without Plex TorBox ownership evidence"
            )
        result = _run("fusermount3", "-uz", self.mountpoint)
        if result.returncode != 0 or _is_disconnected(self.mountpoint):
            raise RuntimeError("Could not detach the stale Plex TorBox mount")
        try:
    ...
    def start(self):
        with self._lock:
            ...
            if not api_key:
                self.detail = "Add a TorBox API key in Plex TorBox"
                return
            webdav_url = str(
    ...
    def health(self) -> dict:
        with self._lock:
            ...
            if running and not mounted and not self.error:
                ...
                self.detail = "A foreign mount is active; Plex TorBox will not take ownership"
            return {
                "online": online,
                "mounted": mounted,
                "owned": self.owned,
